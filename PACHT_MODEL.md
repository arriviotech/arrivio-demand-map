...
**Live deduction behaviour (no fixed anchor).** The per-hotel card does **not** cite a single fixed anchor (the earlier "Wuppertal €185/room" line was removed). For a hotel with no live listing it first looks for **nearby listed hotel-lease comparables** — the tightest radius (30 → 75 → 150 km) that yields ≥2 — and anchors the range to their median €/room, citing the count + radius. If none are nearby it documents the **regional** basis instead: that Bundesland's real **Destatis GENESIS occupancy** × category ADR × the 15–25 % revenue rule. So every estimate is justified by either local real listings or the region's own demand/occupancy data, not by one out-of-area property.
